...
class LeitorCSV:
    ...
    def leitura_csv_e_declaracao_variaveis_dados(self):
        self.__df_agentes_economicos = pd.read_csv('AgentesEconomicosRegulares.csv', encoding='latin1', sep=';')
        self.agnts_economicos = pd.DataFrame(self.__df_agentes_economicos)
        #
        self.__df_atividades_economicas_agentes = pd.read_csv('AtividadesEconomicasAgentesRegulares.csv', encoding='latin1', sep=';')
        self.atividade_economicas_agnts = pd.DataFrame(self.__df_atividades_economicas_agentes)
        #
        self.__df_projetos_renuncia_fiscal = pd.read_csv('ProjetosRenunciaFiscal.csv', encoding='latin1', sep=';') 
        self.proj_renuncia_fiscal = pd.DataFrame(self.__df_projetos_renuncia_fiscal)
        #
        self.__df_obras_nao_publicitaria_fomento_indireto = pd.read_csv('ObrasNaoPubBrasileirasFomentoIndireto.csv', encoding='utf-16be', sep=';')
        self.obras_n_publi_fomento_direto = pd.DataFrame(self.__df_obras_nao_publicitaria_fomento_indireto)
        #
        self.__df_obras_nao_publicitaria_investimento_fsa = pd.read_csv('ObrasNaoPubBrasileirasInvestimentoFsa.csv', encoding='utf-16be', sep=';')
        self.obras_n_publi_investimento_fsa = pd.DataFrame(self.__df_obras_nao_publicitaria_investimento_fsa)
        #
        self.__df_processos_em_prestacao_de_contas = pd.read_csv('ProcessosEmPrestacaoDeContas.csv', encoding='latin1', sep=';')
        self.processos_prestacao_contas =  pd.DataFrame(self.__df_processos_em_prestacao_de_contas)
        #
        self.__df_investidores_projetos_renuncia_fiscal = pd.read_csv('InvestidoresEmProjetosRenunciaFiscal.csv', encoding='latin1', sep=';')
        self.investidores_proj_renuncia_fiscal = pd.DataFrame(self.__df_investidores_projetos_renuncia_fiscal)
        #
        self.__df_produtoras_independentes = pd.read_csv('ProdutorasIndependentes.csv', encoding='latin1', sep=';')
        self.produtoras_independentes = pd.DataFrame(self.__df_produtoras_independentes)
        #
        self.__df_produtores_obras_nao_publicitarias_brasileiras = pd.read_csv('ProdutoresDeObrasNaoPublicitariasBrasileiras.csv', encoding='latin1', sep=';')
        self.produtores_obras_n_publi =  pd.DataFrame(self.__df_produtores_obras_nao_publicitarias_brasileiras)
        #
        self.__df_projetos_contratados_fsa_desembolso = pd.read_csv('ProjetosContratadosFSAeDesembolso.csv', encoding='latin1', sep=';')
        self.proj_fsa_desembolso =  pd.DataFrame(self.__df_projetos_contratados_fsa_desembolso)
        #
        self.__df_relacao_grupos_economicos = pd.read_csv('RelacaoGruposEconomicos.csv', encoding='latin1', sep=';')
        self.relacao_grupos_economicos = pd.DataFrame(self.__df_relacao_grupos_economicos)
        #
        self.__df_salas_e_complexos = pd.read_csv('SalasDeExibicaoEComplexos.csv', encoding='latin1', sep=';')
